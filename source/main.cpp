...
class HealthPack : public Asteroid {
public:
	HealthPack(int w, int h) : Asteroid(w, h) { baseDamage = -5; }
	void Draw() const override {
		Renderer::Instance().DrawPoly(transform.position, 6, 10, transform.rotation);
	}
};

// Shape selector
enum class AsteroidShape { TRIANGLE = 3, SQUARE = 4, PENTAGON = 5, RANDOM = 0, HP = 6 };
...
static inline std::unique_ptr<Asteroid> MakeAsteroid(int w, int h, AsteroidShape shape) {
	...
	case AsteroidShape::HP:
		return std::make_unique<HealthPack>(w, h);
	default: {
...
enum class WeaponType { LASER, BULLET, SZEROKI, RAKIETA, COUNT };
class Projectile {
public:
	...
	void Draw() const {
		...
		else if(type== WeaponType::RAKIETA)
		{
		...
		{
			static constexpr float SZEROKI_LENGTH = 10.f;
			Rectangle lr = { transform.position.x - 2.f, transform.position.y - SZEROKI_LENGTH, 4.f, SZEROKI_LENGTH };
	...
	const std::vector<std::unique_ptr<Asteroid>>& asteroids)
{
	...
	else if (wt == WeaponType::SZEROKI) {
		const float angleOffset = 30.f * (PI / 180.f); 
		for (float angle = -angleOffset; angle <= angleOffset; angle += angleOffset) {
			Vector2 SZEROKIVel = Vector2Rotate(vel, angle);
			result.emplace_back(pos, SZEROKIVel, 8, wt);
		}
	}
...
public:
	...
	float GetFireRate(WeaponType wt) const {
		...
		case WeaponType::BULLET:
			...
			break;
		case WeaponType::RAKIETA:
			return fireRateRAKIETA;
			break;
		default:
			return fireRateLaser;
			break;
...
protected:
	...
	float      spacingLaser;
	float      spacingBullet;
	float      fireRateRAKIETA;
	float      spacingRAKIETA;
};

class PlayerShip :public Ship {
public:
	PlayerShip(int w, int h) : Ship(w, h) {
		texture = LoadTexture("piwo.png");
		...
		while (!WindowShouldClose()) {
			...
			// Render everything
			{
				...
				DrawText(TextFormat("HP: %d", player->GetHP()),
					10, 10, 20, GREEN);
				DrawText(TextFormat("Points: %d", pts),
					10, 30, 20, GREEN);

				const char* weaponName = "";
				switch (currentWeapon) {
				case WeaponType::LASER: weaponName = "LASER"; break;
				case WeaponType::BULLET: weaponName = "NABUJ"; break;
				case WeaponType::RAKIETA: weaponName = "RAKIETA"; break;
				case WeaponType::SZEROKI: weaponName = "SZEROKI"; break;
				default: weaponName = "NIEZNANE"; break;
				}

				const char* asteroidName = "";
				switch (currentShape) {

				case AsteroidShape::TRIANGLE: asteroidName = "TRUJKOT"; break;
				case AsteroidShape::SQUARE: asteroidName = "KWADRAT"; break;
				case AsteroidShape::PENTAGON: asteroidName = "PIECIOKOT"; break;
				case AsteroidShape::HP: asteroidName = "LECZENIE"; break;
				case AsteroidShape::RANDOM: asteroidName = "LOSOWE"; break;
				default: asteroidName = "NIEZNANE"; break; 
				}

				DrawText(TextFormat("Weapon type: %s", weaponName),
					10, 60, 20, BLUE);
				DrawText(TextFormat("Asteroid type: %s", asteroidName),
					10, 80, 20, BLUE);

				if (player->IsAlive() && pts >= 50)
				{
					Renderer::Instance().clear();
					DrawText(TextFormat("WYGRALES!!! \n Liczba punktow: %d", pts), 300, 300, 20, YELLOW);
					currentShape = AsteroidShape::HP;
				}
				else {
...
private:
	...
	static constexpr float C_SPAWN_MAX = 2.0f;
